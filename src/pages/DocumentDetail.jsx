      ...
      <div className="saved-document-layout">
        <div className="saved-document-main">
          ...
          <section className="empty-work-card generated-task-workspace">
            ...
            {relatedTasks.length > 0 && <div className="document-generated-list">{relatedTasks.map((task) => <div key={task.id} className={task.status === 'Completed' ? 'task-completed' : 'task-pending'}>{task.status === 'Completed' ? <CheckCircle2 /> : <Clock3 />}<span><strong>{task.title}</strong><small>{task.status} · {task.priority} priority</small></span></div>)}</div>}
...
function AiReviewWorkspace({ analysis, busy, serverError, onConfirm, onReject }) {
  ...
  return <section className="ai-review-workspace" id="document-ai-review">
    <header className={`ai-review-header status-${analysis.reviewStatus || 'pending_review'}`}><span className="ai-hero-icon">{confirmed ? <CheckCircle2 /> : <Bot />}</span><div><small>Analysis status</small><h2>{statusLabel}</h2><p>{confirmed ? 'Your corrections have been saved.' : rejected ? 'The AI suggestion was rejected. Analyze again to create a new review.' : 'Tasks that passed the safety gate may already exist. Review and edit this analysis whenever correction is needed.'}</p></div><span className="review-status-pill"><i />{confirmed ? 'Confirmed' : rejected ? 'Rejected' : 'Pending review'}</span></header>
    {(localError || serverError) && <div className="form-error" role="alert">{localError || serverError}</div>}
    <div className="ai-review-form">
      <ReviewSection icon={<Bot />} variant="summary" title="AI generated summary" hint="Edit the summary so it matches the source document.">{editable ? <textarea value={draft.summary} disabled={!editable || busy} maxLength={5000} rows="4" onChange={(event) => setDraft((current) => ({ ...current, summary: event.target.value }))} /> : <p className="review-static review-summary-text">{draft.summary || 'No summary provided.'}</p>}</ReviewSection>
      <ReviewSection icon={<Layers3 />} variant="category" title="Category" hint="Choose the closest document category."><select value={draft.category} disabled={!editable || busy} onChange={(event) => setDraft((current) => ({ ...current, category: event.target.value }))}><option value="">No category</option>{documentCategories.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></ReviewSection>
      <ReviewSection icon={<CalendarDays />} variant="dates" title="Important dates" hint="Remove dates that are not explicitly supported by the document." action={editable && <button onClick={() => setDraft((current) => ({ ...current, importantDates: [...current.importantDates, { date: '', description: '' }] }))}>+ Add date</button>}><div className="review-edit-list">{draft.importantDates.map((item, index) => <div className="review-edit-row date" key={index}>{editable ? <input aria-label="Date" placeholder="Date" value={item.date} disabled={!editable || busy} onChange={(event) => updateList('importantDates', index, { ...item, date: event.target.value })} /> : <p className="review-static review-date-value">{item.date}</p>}{editable ? <input aria-label="Date description" placeholder="What happens on this date?" value={item.description} disabled={!editable || busy} onChange={(event) => updateList('importantDates', index, { ...item, description: event.target.value })} /> : <p className="review-static">{item.description}</p>}{editable && <button aria-label="Remove date" onClick={() => removeList('importantDates', index)}>×</button>}</div>)}{!draft.importantDates.length && <p className="review-none">No important dates identified.</p>}</div></ReviewSection>
      {['keyInformation', 'risksOrConsequences'].map((field) => <ReviewSection key={field} variant={field === 'keyInformation' ? 'information' : 'risks'} icon={field === 'keyInformation' ? <FileText /> : <ShieldCheck />} title={field === 'keyInformation' ? 'Key information' : 'Risks & consequences'} hint={field === 'keyInformation' ? 'Keep only important facts found in the document.' : 'Review possible outcomes carefully.'} action={editable && <button onClick={() => setDraft((current) => ({ ...current, [field]: [...current[field], ''] }))}>+ Add item</button>}><div className="review-edit-list">{draft[field].map((item, index) => <div className="review-edit-row" key={index}>{editable ? <input value={item} aria-label={`${field} item`} disabled={!editable || busy} onChange={(event) => updateList(field, index, event.target.value)} /> : <p className="review-static">{item}</p>}{editable && <button aria-label="Remove item" onClick={() => removeList(field, index)}>×</button>}</div>)}{!draft[field].length && <p className="review-none">{field === 'keyInformation' ? 'No key information identified.' : 'No risks or consequences identified.'}</p>}</div></ReviewSection>)}
      <ReviewSection icon={<CheckCircle2 />} variant="actions" count={`${draft.extractedActions.length} ${draft.extractedActions.length === 1 ? 'action' : 'actions'}`} title="Suggested actions" hint="Confirm that every action is actually required. Add a due date only when the document states one." action={editable && <button onClick={() => setDraft((current) => ({ ...current, actionRequired: true, extractedActions: [...current.extractedActions, { title: '', description: '', priority: 'medium', dueDate: '' }] }))}>+ Add action</button>}><div className="review-edit-list">{!draft.extractedActions.length && <div className="no-action-detected"><CheckCircle2 /><div><strong>No actionable tasks detected</strong><p>This document appears informational. Add an action only if it contains a real obligation you need to complete.</p></div></div>}{draft.extractedActions.map((item, index) => <div className="review-action-card" key={index}><span className="review-action-marker" aria-hidden="true">{confirmed ? <CheckCircle2 /> : <Clock3 />}</span><div>{editable ? <input aria-label="Action title" placeholder="Action title" value={item.title} disabled={!editable || busy} onChange={(event) => updateList('extractedActions', index, { ...item, title: event.target.value })} /> : <strong className="review-static review-action-title">{item.title}</strong>}<select className={`review-priority priority-${item.priority}`} aria-label="Action priority" value={item.priority} disabled={!editable || busy} onChange={(event) => updateList('extractedActions', index, { ...item, priority: event.target.value })}>{['low', 'medium', 'high'].map((priority) => <option key={priority}>{priority}</option>)}</select>{editable ? <input aria-label="Action due date" type="text" placeholder="YYYY-MM-DD or ISO date/time" value={item.dueDate || ''} disabled={!editable || busy} onChange={(event) => updateList('extractedActions', index, { ...item, dueDate: event.target.value })} /> : <span className="review-static review-action-due">{item.dueDate || 'No due date'}</span>}{editable && <button aria-label="Remove action" onClick={() => removeList('extractedActions', index)}>×</button>}</div>{editable ? <textarea aria-label="Action description" placeholder="Action details" value={item.description} disabled={!editable || busy} rows="2" onChange={(event) => updateList('extractedActions', index, { ...item, description: event.target.value })} /> : <p className="review-static review-action-description">{item.description || 'No additional details.'}</p>}</div>)}</div></ReviewSection>
    </div>
...
function ReviewSection({ icon, title, hint, action, children, variant = 'neutral', count }) {
  return <section className={`review-section review-${variant}`}><header><span>{icon}</span><div><h3>{title}{count && <span className="review-count">{count}</span>}</h3><p>{hint}</p></div>{action}</header><div className="review-section-body">{children}</div></section>;
}
