import { BellRing, CalendarDays, Check, ChevronDown, CircleDot, Folder, FolderOpen, Pencil, Plus, Trash2 } from 'lucide-react';
...
export default function Tasks() {
  ...
  return <>
    <PageHeader title="My Tasks" description="Actions generated from your documents and tasks you create yourself." action={<Button onClick={() => { setFormError(''); setEdit({ id: 'new', title: '', priorityOverride: '', status: 'Pending' }); }}><Plus size={16} />New task</Button>} />
    ...
    {tasksLoading ? <section className="panel"><Skeleton lines={6} /></section> : tasksError ? <section className="panel"><EmptyState title="Tasks could not be loaded" text={tasksError} action={<Button onClick={reloadTasks}>Try again</Button>} /></section> : filtered.length ? <section className="task-kanban" aria-label="Task board">{lanes.map((lane) => <div className={`task-lane lane-${lane.tone}`} key={lane.key}><header><div><h2>{lane.title}</h2><span>{lane.items.length}</span></div><p>{lane.description}</p></header><div className="task-lane-body">{lane.items.length ? lane.items.map((item) => item.kind === 'group' ? <TaskGroupCard group={item} key={item.id} onEdit={(task) => { setFormError(''); setEdit(task); }} onDelete={setDeleting} onComplete={(task) => completeTask(task.id).catch((error) => notify(getErrorMessage(error)))} onReminder={setReminderTask} /> : <TaskKanbanCard task={item.task} key={item.id} onEdit={() => { setFormError(''); setEdit(item.task); }} onDelete={() => setDeleting(item.task)} onComplete={() => completeTask(item.id).catch((error) => notify(getErrorMessage(error)))} onReminder={() => setReminderTask(item.task)} />) : <div className="task-lane-empty"><CircleDot /><span>No tasks here</span></div>}</div></div>)}</section> : <section className="panel"><EmptyState title="Nothing needs your attention" text="You're all caught up in this view." /></section>}
...
function TaskGroupCard({ group, onEdit, onDelete, onComplete, onReminder }) {
  ...
  return <article className={`task-group-card ${expanded ? 'is-expanded' : 'is-collapsed'}`} style={{ '--folder-accent': accent.color, '--folder-tint': accent.tint }}>
    <button className="task-group-header" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded}>
      ...
      <span className="task-group-heading"><small>{group.category} · DOCUMENT TASKS</small><strong>{group.title}</strong><em>{completed} of {group.tasks.length} tasks completed</em></span>
    ...
    <div className="task-group-items">{visibleTasks.map((task) => <div className={`task-group-item ${task.status === 'Completed' ? 'is-completed' : ''}`} key={task.id}>
      ...
      <span className="task-priority-display" title={task.priorityReasons?.join(' • ')}><PriorityBadge priority={task.priority} />{task.priorityOverride && <small>Override</small>}</span>
      <div className="task-group-item-actions"><button onClick={() => onReminder(task)} aria-label={`Set reminder for ${task.title}`}><BellRing /></button><button onClick={() => onEdit(task)} aria-label={`Edit ${task.title}`}><Pencil /></button><button onClick={() => onDelete(task)} aria-label={`Delete ${task.title}`}><Trash2 /></button></div>
    </div>)}</div>
