...
import { Button, ConfirmDialog, EmptyState, PageHeader, SearchBox, Skeleton } from '../components/UI';
...
export default function Documents() {
  ...
  const { documents, documentsLoading, documentsError, reloadDocuments, deleteDocument, notify } = useApp();
  ...
  const confirmDelete = async () => {
    ...
    try { await deleteDocument(deleting.id); setDeleting(null); notify('Document deleted'); }
  ...
  return <><PageHeader title="Documents" description={`${documents.length} saved record${documents.length === 1 ? '' : 's'} · ${documents.filter((item) => item.sourceType === 'pdf').length} PDFs · ${documents.filter((item) => item.sourceType === 'image').length} images`} action={<Button onClick={() => nav('/app/add')}><Plus size={16} />Add Information</Button>} />
    <div className="toolbar document-toolbar"><SearchBox value={query} onChange={setQuery} placeholder="Search title, filename, category or content…" /><div className="tool-filters"><label><span className="sr-only">Source type</span><select value={source} onChange={(event) => setSource(event.target.value)}>{sourceOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label><span className="sr-only">Sort documents</span><select value={sort} onChange={(event) => setSort(event.target.value)}>{sortOptions.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><div className="segmented"><button aria-label="Grid view" aria-pressed={view === 'grid'} className={view === 'grid' ? 'active' : ''} onClick={() => setView('grid')}><Grid2X2 size={15} /></button><button aria-label="List view" aria-pressed={view === 'list'} className={view === 'list' ? 'active' : ''} onClick={() => setView('list')}><List size={16} /></button></div></div></div>
    <div className="tabs document-category-tabs"><button className={category === 'all' ? 'active' : ''} onClick={() => setCategory('all')}>All</button>{documentCategories.map(([value, label]) => <button className={category === value ? 'active' : ''} key={value} onClick={() => setCategory(value)}>{label}</button>)}</div>
    {documentsLoading ? <div className="documents grid" aria-label="Loading documents">{[1, 2, 3].map((item) => <div className="panel" key={item}><Skeleton lines={4} /></div>)}</div> : documentsError ? <EmptyState title="Unable to load documents." text="Check your connection and try again." action={<Button onClick={reloadDocuments}>Try again</Button>} /> : list.length ? <div className={`documents ${view}`}>{list.map((document) => <DocumentCard key={document.id} doc={document} view={view} onDelete={setDeleting} />)}</div> : <EmptyState title={documents.length ? query.trim() ? `No documents match “${query.trim()}”` : 'No matching documents' : 'No documents yet'} text={documents.length ? 'Change your search or clear the active filters.' : 'Add your first piece of information to get started.'} action={documents.length && hasFilters ? <Button variant="secondary" onClick={clearFilters}>Clear search and filters</Button> : !documents.length ? <Button onClick={() => nav('/app/add')}>Add Information</Button> : undefined} />}
    <ConfirmDialog open={Boolean(deleting)} onClose={() => { if (!deleteBusy) { setDeleting(null); setDeleteError(''); } }} onConfirm={confirmDelete} title="Delete document?" text={deleteError || `This will permanently remove “${deleting?.title || 'this document'}”${deleting?.sourceType === 'pdf' || deleting?.sourceType === 'image' ? ' and its original uploaded file' : ''}.`} confirmLabel="Delete" busy={deleteBusy} /></>;
}
