  ...
  const configured = (name) =>
    name === 'groq'
      ...
      : name === 'gemini'
        ...
        : false;
  const primary = configured(config.provider)
    ? config.provider
    : configured(config.fallbackProvider)
  ...
  const call = async (name) => {
    ...
    try {
      const result = await Promise.race([
        providers[name]({
          ...request,
          client: name === 'groq' ? providerOptions.client : providerOptions.geminiClient,
        }),
        new Promise((_, reject) => {
          timer = setTimeout(
            () => reject(new AiError(AI_ERROR_CODES.TIMEOUT, { statusCode: 504 })),
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development')
        console.info('[AI]', {
          ...
          fallbackUsed: true,
          inputCharacters: request.userPrompt.length,
          durationMs: Date.now() - started,
        });
      return result;
    } catch (fallbackError) {
      if (
        primaryError.code === AI_ERROR_CODES.RATE_LIMITED &&
        fallbackError.code === AI_ERROR_CODES.RATE_LIMITED
      )
        throw new AiError(AI_ERROR_CODES.RATE_LIMITED, { statusCode: 429 });
      if (
        primaryError.code === AI_ERROR_CODES.TIMEOUT &&
        fallbackError.code === AI_ERROR_CODES.TIMEOUT
      )
        throw new AiError(AI_ERROR_CODES.TIMEOUT, { statusCode: 504 });
      if (fallbackError.code === AI_ERROR_CODES.INVALID_RESPONSE)
        throw new AiError(AI_ERROR_CODES.INVALID_RESPONSE, { statusCode: 502 });
